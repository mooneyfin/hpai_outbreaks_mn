...
# The meteorological effect at lag 8-14 days, shown separately for cell-days with high and low
...
#
# Only the 8-14 day window is drawn. It is the window the hypothesis named ("the fortnight
# before"), and the 0-7 day estimates sit in Table S7a. Anseriformes is not a row here: it is
# the modifier, not an exposure being modified.
#
...
dat <- rbindlist(lapply(names(GRP), function(g)
  cbind(ip[Window == "8-14 days", .(Kind, Panel, Exposure, n)], group = GRP[[g]], parse_est(ip[Window == "8-14 days"][[g]]))))
...
one_block <- function(kind, keep_xlab) {
  ggplot(dat[Kind == kind], aes(RR, Exposure, colour = group, shape = group)) +
    ...
    labs(x = if (keep_xlab) "Rate ratio per +0.5 SD increase in exposure, lag 8-14 days" else NULL,
...
ggsave_spark(file.path(figures_main_folder, "figure4_interaction.png"), fig, width = 12, height = 8.5)
cat("wrote figure4_interaction.png\n")
print(dcast(dat[Exposure == "Temperature"], Kind + Panel ~ group, value.var = "RR"))
