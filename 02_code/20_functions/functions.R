...
############ FIGURE HELPERS (SPARK Lab NYC conventions) ############
# Pull these palettes and theme helpers when producing any figure in this
# project. The defaults mirror the lab figures style guide (Avenir typography;
# theme_classic for plots / theme_void for maps; bottom-justified legend).
...
spark_diverging_rate_colors <- c(low  = "#2D6FA3",
                                 mid  = "#F2EBCF",
                                 high = "#C9405A")
...
# 1d. Age-group palette (full 18-level sequential — use even when plotting a subset)
...
# 2a. Standard SPARK theme for non-map plots (line, bar, forest)
...
# 2b. SPARK theme for maps (US Albers Equal Area Conic, EPSG:5070, is assumed
#     upstream — set crs via sf::st_transform before plotting)
