---
title: "Climate Anomaly Models"
author: "Fintan Mooney"
date: "2025-10-16"
output: html_document
---

# GAMs using climate anomaly z-scores (relative to 10yr and 25yr means) as predictors of HPAI outbreaks... an alternative modeling framework to the DLNM approach!
...
## Read in the weekly climate-anomaly panel

```{r, read in timeseries df}

# This Rmd is sensitivity-only and operates at WEEKLY resolution (climate
# anomalies are derived from 10/15/20/25-year weekly climatologies). Run
# 02_code/2b_data_exploration/b_00_climate_anomaly_setup.Rmd first to produce this object.
fishnet_timeseries_clean <- readRDS(file.path(objects_folder, "climate_anomaly_panel.RDS"))
```
...
ggplot2::ggsave(
  filename = file.path(figures_sensitivity_folder, "figure_temperature_anomaly_25yr10yr_comparision.png"),
...
# Create the 3×2 grid of anomaly plots
panel_anomalies <- patchwork::wrap_plots(smooth_plots, ncol = 3) +
plot_layout(axis_titles = "collect")

panel_anomalies <- panel_anomalies +
  patchwork::plot_annotation(
    title   = "Nonlinear Relative Risk of HPAI Spillover Events vs Environmental Anomalies",
...
fishnet_path <- file.path(here("data", "processed"), "HPAIfishnet_zone_dissolved_10km.shp")
...
p_spatial <- ggplot2::ggplot() +
  ...
  ggplot2::labs(
    title = "Spatial Smooth: Baseline Relative Risk of HPAI Spillover Events",
...
p_season <- ggplot2::ggplot(season_effects, ggplot2::aes(x = Season, y = RR)) +
  # point for estimate
  ggplot2::geom_point(size = 3, shape = 21, fill = "black") +
  # vertical error bars
  ggplot2::geom_errorbar(ggplot2::aes(ymin = lower, ymax = upper), width = 0.15, linewidth = 0.6) +
  # dashed baseline at RR = 1 (Winter reference)
  ggplot2::geom_hline(yintercept = 1, linetype = "dashed") +
  ...
  ggplot2::labs(
    title = "Seasonal Baseline Effects on HPAI Spillover Risk",
    x = "Season",
...
final_panel_25yr <-
  (panel_anomalies /
   (p_spatial | p_season)) +
  patchwork::plot_annotation(
    caption = "Negative-binomial GAM: The association between climate anomalies in 2022 (compared to 25yr mean) on HPAI spillover risk in commercial and backyard farms.",
...
final_panel_10yr <-
  (panel_anomalies /
   (p_spatial | p_season)) +
  patchwork::plot_annotation(
    caption = "Negative-binomial GAM: The association between climate anomalies in 2022 (compared to 10yr mean) on HPAI spillover risk in commercial and backyard farms.",
    theme = ggplot2::theme(plot.caption = ggplot2::element_text(size = 9, hjust = 0))
  )
...
ggplot2::ggsave(
  filename = file.path(figures_sensitivity_folder, "figure_25yr_climate_anomaly_model.png"),
...
ggplot2::ggsave(
  filename = file.path(figures_sensitivity_folder, "figure_10yr_climate_anomaly_model.png"),
...
message("Figures successfully exported to: ", figures_sensitivity_folder)
